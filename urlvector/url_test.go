...
func BenchmarkNetUrl_ParseUrl(b *testing.B) {
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
...
func BenchmarkNetUrl_ParseQuery(b *testing.B) {
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		u, err := url.Parse(fastconv.B2S(url0))
		if err != nil {
...
func BenchmarkNetUrl_ModHost(b *testing.B) {
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		u, err := url.Parse(fastconv.B2S(url0))
		if err != nil {
...
func BenchmarkNetUrl_ModQuery(b *testing.B) {
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		u, err := url.Parse(fastconv.B2S(url0))
		if err != nil {
