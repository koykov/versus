...
func (i2 L3Inspector) GetTo(src any, buf *any, path ...string) (err error) {
	...
	var x *inspector2.L3
	_ = x
	if p, ok := src.(**inspector2.L3); ok {
		x = *p
	} else if p, ok := src.(*inspector2.L3); ok {
		x = p
	} else if v, ok := src.(inspector2.L3); ok {
		x = &v
	} else {
		return
	}
...
func (i2 L3Inspector) SetWithBuffer(dst, value any, buf inspector.AccumulativeBuffer, path ...string) error {
	...
	var x *inspector2.L3
	_ = x
	if p, ok := dst.(**inspector2.L3); ok {
		x = *p
	} else if p, ok := dst.(*inspector2.L3); ok {
		x = p
	} else if v, ok := dst.(inspector2.L3); ok {
		x = &v
	} else {
		return nil
	}
...
func (i2 L3Inspector) DeepEqualWithOptions(l, r any, opts *inspector.DEQOptions) bool {
	...
	if lp, ok := l.(**inspector2.L3); ok {
		lx, leq = *lp, true
	} else if lp, ok := l.(*inspector2.L3); ok {
		lx, leq = lp, true
	} else if lp, ok := l.(inspector2.L3); ok {
		lx, leq = &lp, true
	}
	if rp, ok := r.(**inspector2.L3); ok {
		rx, req = *rp, true
	} else if rp, ok := r.(*inspector2.L3); ok {
		rx, req = rp, true
	} else if rp, ok := r.(inspector2.L3); ok {
		rx, req = &rp, true
	}
...
func (i2 L3Inspector) countBytes(x *inspector2.L3) (c int) {
	c += len(x.S)
	return c
}

func (i2 L3Inspector) cpy(buf []byte, l, r *inspector2.L3) ([]byte, error) {
...
func (i2 L3Inspector) Reset(x any) error {
	var origin *inspector2.L3
	_ = origin
	switch x.(type) {
	case inspector2.L3:
		return inspector.ErrMustPointerType
	case *inspector2.L3:
		origin = x.(*inspector2.L3)
	case **inspector2.L3:
		origin = *x.(**inspector2.L3)
	default:
		return inspector.ErrUnsupportedType
